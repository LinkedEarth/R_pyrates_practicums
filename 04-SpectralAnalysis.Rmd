...
![](http://linked.earth/FROGS/images/Pyrateslogo.jpg){width=400}
...
### Packages
In addition to the standard R packages (tidyverse, lubridate), this tutorial showcases: 
- [imputeTS](https://cran.r-project.org/web/packages/imputeTS/index.html) for time series imputation
- [TSA](https://cran.r-project.org/web/packages/TSA/index.html) for time series analysis, including spectral analysis
- [stlplus](https://cran.r-project.org/web/packages/stlplus/index.html) for STL decomposition
- [biwavelet](https://cran.r-project.org/web/packages/biwavelet/index.html) for wavelet analysis
...
toScale <- filter(mtm3, between(Period,0.2,100))
...
```{r}
# Log-transform both axes
toScale <- filter(mtm3, between(Period,2,100))
log_period <- log10(toScale$Period)
log_spec <- log10(toScale$Power)
...
We return to the original series and apply 2 techniques to obtain the spectrum, keeping gaps in the series.

1. Lomb-Scargle periodogram:
```{r}
# Convert to numeric vector and remove NAs
dis_vec <- as.numeric(df$`discharge (cf/s)`)
dis_vec <- dis_vec[!is.na(dis_vec)]

# Create time vector (in days)
time_vec <- as.numeric(df$datetime[!is.na(df$`discharge (cf/s)`)]) - as.numeric(df$datetime[1])

# Compute Lomb-Scargle periodogram
lomb <- astrochron::ls(dis_vec, times = time_vec, ofac = 4, scaleType = "period", plot = FALSE)

# Plot
plot(lomb$scanned, lomb$spec, type = "l", log = "xy",
     xlab = "Period [days]", ylab = "PSD", main = "Lomb-Scargle Periodogram")
```

2. Wavelet analysis using the Morlet wavelet:
```{r}
